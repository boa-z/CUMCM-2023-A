...
df = pd.read_excel('D:\桌面\CUMCM2023Problems\CUMCM2023Problems\A题\附件.xlsx')
...
while current_date <= end_date:
    ...
    for time in times:
        ...
        # 计算平均值并添加到平均时间列表
        avg_time = sum(heli_output) / len(heli_output)
        avg_times.append(avg_time)
        print(local_time, avg_time)
    # 计算5个时间的平均值的平均值并添加到月平均列表
    avg_month.append(sum(avg_times) / len(avg_times))
    print(avg_month)
