...
# def if_mine(hhday):
#     # ~~当天消耗 + 第二天消耗 对比 当天行走；如果行走消耗更少则行走~~
#     # 计算当天消耗量价值 对比 挖矿收益，如果挖矿收益更高则挖矿
#     temp_water = base_consume_water[get_weather(weather[hhday-1])]*3*base_water_price
#     temp_food = base_consume_food[get_weather(weather[hhday-1])]*3*base_food_price
#     temp_money = 1000
#     print(temp_water,temp_food,temp_money)
#     if temp_water+temp_food>temp_money:
#         return 0
#     else:
#         return 1
...
def possess_c(cur_water,cur_food,cur_money,cur_day, log):
    ...
    newlog += "At Day " + str(e) + ": " + "Move Mine water and food " + str(temp_water2) + " " + str(
        temp_food2) + "\n"
    possess_k(temp_water2, temp_food2, temp_money, e,newlog)
...
def possess_k(cur_water,cur_food,cur_money,cur_day,log):
...
def train():
    i=0
    for init_water in range(175,200):
        for init_food in range(330,360):
            i+=1
